  ...
  <div class="user-table-div">
    <scroller style="position: absolute;width: 100%;height: 90%" :on-infinite="infinite"
      ...
      <main style="width: 100%;height: 100%">
        <group>
          <divider>总数:{{totalNum}},当前{{this.unitData.length}}</divider>
          <div v-for="(item, index) in unitData">
            <cell :title="item.unitName"
                  :inline-desc="item.unitNo"
  ...
  export default {
    ...
    methods: {
      infinite: function (done) {
        this.pageNum++;
        this.getUnitData(false, done);
      },
      refresh: function (done) {
        this.pageNum = 1;
        this.getUnitData(true, done);
      },
      ...
      doSearch: function () {
        this.getUnitData(true, null);
        this.showSearchDialog = false;
      },
      getUnitData(refresh, callback) {
        if (this.onFetching) {
          return;
        }
        let params = {
        ...
        this.$http.post(this.API_DYNY.GMS.getUnitList, params, {emulateJSON: true}).then(res => {
          this.totalNum = res.body.total_num;
          this.pageNum = res.body.page_num;
          if (!refresh) {
            this.unitData = this.unitData.concat(res.body.data);
          } else {
